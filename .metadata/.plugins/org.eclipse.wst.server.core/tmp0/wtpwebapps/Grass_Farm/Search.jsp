...
<body>
	...
	<%}%>
		...
		getmyrepo
 	<%
	ArrayList<Bbs> myrepolist=bbsDAO.getmyrepo((String)session.getAttribute("userID"),search);
	for(int i=0; i<myrepolist.size(); i++){
	%>
		<div class="container">
		<div class="col-md-12">
    		<div class="card b-1 hover-shadow mb-20">
        		<div class="media card-body"> 
           			<div class="media-body">
                			...
                			<div class="mb-2">
                				<span class="fs-20 pr-16" type="submit" onclick="location.href='SearchUser.jsp?id=<%=myrepolist.get(i).getUserID()%>'" ></span>
               				</div>
               				@<%=myrepolist.get(i).getUserID()%></input><br>
               			
                			<p class="fs-14 text-fade mb-12"><i class="fa fa-map-marker pr-1">
                			</i><%=myrepolist.get(i).getBbsTitle()%></p>
            		</div>
        		</div>
        		<footer class="card-footer flexbox align-items-center">
            	<div>
               		<strong>Applied on:</strong>
                	<span><%=myrepolist.get(i).getBbsDate()%></span>             
            	</div>
            	<div class="media-right text-right d-none d-md-block">
